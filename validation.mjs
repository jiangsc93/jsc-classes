import { arrayLike as arrayLike_1 } from './node_modules/jsc-utils/array.mjs';
import { objectEach as objectEach_1, objectAssign as objectAssign_1 } from './node_modules/jsc-utils/object.mjs';
import { t as type } from './node_modules/jsc-utils/type.mjs';
import { isUrl as isUrl_1, isEmail as isEmail_1, isPhone as isPhone_1, isIpV4 as isIpV4_1, isIdNo as isIdNo_1, isInteger as isInteger_1, isFloat as isFloat_1, isNumerical as isNumerical_1, isDigit as isDigit_1 } from './node_modules/jsc-utils/validator.mjs';
import { stringAssign as stringAssign_1 } from './node_modules/jsc-utils/string.mjs';
import Fault from './fault.mjs';
...
const messageGetter = (messageFunc, message) => {
    ...
    if (type.isString(message))
        return (value, source, rule, schema, msgData) => stringAssign_1(message, msgData);
    if (type.isString(messageFunc))
        return (value, source, rule, schema, msgData) => stringAssign_1(messageFunc, msgData);
    return messageFunc;
};
...
class Validation {
    ...
    exportDesc(field) {
        ...
        objectEach_1(_schema, (desc, _field) => {
            if (_field === field) {
                // eslint-disable-next-line @typescript-eslint/ban-ts-ignore
                // @ts-ignore
                found = desc;
    ...
    type(type$1, message) {
        this._createRule('type', {
            ...
            message: messageGetter((value, source, rule, schema, msgData) => {
                ...
                return stringAssign_1('${field}必须是' + type + '类型', msgData);
            }, message)
        });
        this._pushRule();
        return this;
    ...
    format(format, message) {
        this._createRule('format', {
            ...
            message: messageGetter((value, source, rule, schema, msgData) => {
                ...
                return stringAssign_1('${field}必须是' + type + '格式', msgData);
            }, message)
        });
        this._pushRule();
        return this;
    ...
    same(field, message) {
        this._createRule('same', {
            ...
            message: messageGetter((value, source, rule, schema, msgData) => {
                ...
                return stringAssign_1('${field}必须与' + target + '相同', msgData);
            }, message)
        });
        this._pushRule();
        return this;
    ...
    async _validate(source, options) {
        const faults = [];
        const opts = objectAssign_1({}, defaults, options);
        ...
        const validate = async (field, value, rule, desc, msgData) => {
            ...
            const pushFault = (message) => {
                let realMessage;
                if (type.isString(message))
                    realMessage = stringAssign_1(message, msgData);
                else
...
const importSchema = (schema) => {
    ...
    objectEach_1(schema, (asyncValidatorRules, field) => {
        v.field(field);
...
export { MEMBER_MESSAGES, Validation as default, defaults, importRules, importSchema, va };
