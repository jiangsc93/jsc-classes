...
class Events {
    constructor(options) {
        this[_options] = object.objectAssign({}, eventsDefaults, options);
        this[_eventsStore] = {};
    ...
    on(type$1, listener) {
        let _type;
        let _listener;
        if (type.type.isFunction(type$1)) {
            _listener = type$1;
     ...
     */
    off(type$1, listener) {
        if (type.type.isString(type$1) && type.type.isFunction(listener))
            this[_offOne](type$1, listener);
        else if (type.type.isString(type$1))
    ...
    [_offOne](type, listener) {
        ...
        array.arrayRemove(store, (val) => val === listener);
    }
    [_offBatch](type) {
    ...
    [_dispatchType](pipable, type, ...payloads) {
        ...
        array.arrayEach(store.slice(), (listener) => {
            if (pipable) {
                // 事件传递
                array.arrayEach(this[_eventsPipeList].slice(), (piper) => {
    ...
    createChannel(id, options) {
        ...
        const { source, target, extend = false, inherit = false } = object.objectAssign({}, options);
        if (id === source) {
    ...
    [_dispatchChannel](imcommingChannel, type$1, ...payloads) {
        ...
        const isEmpty = (any) => type.type.isNull(any) || type.type.isUndefined(any);
